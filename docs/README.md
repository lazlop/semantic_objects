...
Welcome to the **Semantic Objects** library documentation! This library connects ontologies for the built environment (particularly ASHRAE Standard 223P) to the tools built on top of them, through a single Python class layer.

The `semantic_objects.s223` classes aren't hand-written - they're generated by ingesting the real ontology's SHACL shapes (see `src/semantic_objects/ingest/`). Hand-written classes remain the override layer for customization that isn't derivable from SHACL.
